...
- ⚠️ **CI 只跑 go 目標**：契約測試已接進 CI 的 `infra` job（重用同一組容器），
  但 java 目標需要團隊 repo 的 wallet-service 與 PostgreSQL，只能在本機跑。
  也就是說 **CI 綠只證明 Go 版沒有回歸，不證明兩版仍然等價**——
  「兩版等價」這件事到目前為止是**手動、按需**驗證的。
